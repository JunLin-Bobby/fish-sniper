/**
 * PKCE + state helpers for the Google OAuth flow.
 *
...
const GOOGLE_OAUTH_PKCE_VERIFIER_SESSION_KEY =
  'google_oauth_pkce_verifier_v1'
const GOOGLE_OAUTH_PKCE_STATE_SESSION_KEY =
  'google_oauth_pkce_state_v1'
const GOOGLE_OAUTH_PKCE_REDIRECT_URI_SESSION_KEY =
  'google_oauth_pkce_redirect_uri_v1'
...
export function persistPendingPkceVerifierAndStateToSessionStorage(options: {
  ...
  window.sessionStorage.setItem(
    GOOGLE_OAUTH_PKCE_VERIFIER_SESSION_KEY,
    options.codeVerifier,
  )
  window.sessionStorage.setItem(
    GOOGLE_OAUTH_PKCE_STATE_SESSION_KEY,
    options.state,
  )
  window.sessionStorage.setItem(
    GOOGLE_OAUTH_PKCE_REDIRECT_URI_SESSION_KEY,
    options.redirectUri,
  )
}
...
export function readAndClearPendingPkceFlowStateFromSessionStorage(): PendingPkceFlowState | null {
  ...
  window.sessionStorage.removeItem(GOOGLE_OAUTH_PKCE_VERIFIER_SESSION_KEY)
  window.sessionStorage.removeItem(GOOGLE_OAUTH_PKCE_STATE_SESSION_KEY)
  window.sessionStorage.removeItem(GOOGLE_OAUTH_PKCE_REDIRECT_URI_SESSION_KEY)
