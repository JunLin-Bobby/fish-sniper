...
def test_resolve_api_key_gemini_from_settings() -> None:
    settings = FishSniperBackendSettings(gemini_api_key="secret-gemini")
    assert (
...
def test_resolve_api_key_openai_from_settings() -> None:
    settings = FishSniperBackendSettings(openai_api_key="secret-openai")
    assert (
...
def test_resolve_api_key_missing_raises() -> None:
    settings = FishSniperBackendSettings(gemini_api_key=None)
...
async def test_router_delegates_to_gemini_adapter(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    settings = FishSniperBackendSettings(gemini_api_key="test-key")
    response = MagicMock()
    response.text = "router-ok"
    sdk_client = MagicMock()
    sdk_client.aio.models.generate_content = AsyncMock(return_value=response)

    def _fake_build(*, model_config: ModelConfig, backend_settings: FishSniperBackendSettings):
        _ = backend_settings
