...
def _embed_log_text_or_return_none_on_transient_failure(
    *,
    ...
    bubbles up so misconfigurations turn into a 503 + loud log instead of being
    silently degraded to ``embedding_status='pending'`` forever.
    """
...
def _raise_logs_database_unavailable(exc: FishSniperPersistenceUnavailableError) -> None:
    """Legacy raise path retained for GET/DELETE (envelope upgrade is in POST/PATCH)."""
...
def _build_logs_database_unavailable_response() -> JSONResponse:
    """503 envelope used by POST/PATCH after a single retry."""
